...
*/
const MyPosts: React.FC<MyPostsPropsType> = React.memo(({postsData, addPost}) => { /*Изначально это был классовый компонент и
он отрисовывался несколько лишних раз. Но, вроде, если использовать "connect", то лишних отрисовок не будет. В любом
случае, мы сделали этот компонент функциональным, чтобы избежать этих лишних перерисовок. Для этого мы и использовали
"React.memo()" (вроде это HOC), что содержит оптимизацию этих перерисовок, чтобы обвернуть этим наш функциональный
компонент.
