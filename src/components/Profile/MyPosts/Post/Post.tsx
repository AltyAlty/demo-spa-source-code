...
*/
const Post: React.FC<PropsType> = ({avatar, message, likes}) => { /*Указываем какие именно "props"
мы получаем, чтобы не писать далее "props.avatar", "props.message" и так далее:
...
компоненте имеют тип "PropsType".*/
    /*
    ...
    */
    return (
        ...
        которое будет отрисовываться в компоненте "MyPosts" при помощи метода "map".*/}
            <img src={avatar} alt=''/> {/*Пост пользователя в его профиле содержит аватар пользователя, ссылка
            на который берется из параметров "props".*/}

            {message} {/*Также пост пользователя в его профиле содержит текст этого поста, которое берется
            из параметров "props".*/}

            <div><span>likes: {likes}</span></div> {/*А также пост пользователя в его профиле содержит
...
export default Post; /*Экспортируем компонент "Post" по default и будем его использовать в нашем проекте под именем
"Post", экспорт необходим для импорта.*/