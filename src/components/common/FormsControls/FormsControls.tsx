...
const FormControl: React.FC<FormControlPropsType> =
    ...
    Только здесь может быть проблема, если мы не касались какого-то поля, то нам разрешат отправить, например, пустой
    пост на странице профиля. Этот объект "meta" с мета-данными придет к нам сверху из элемента "Field". Указали при
    помощи "React.FC<>", что "props" в этом функциональном компоненте имеют тип "FormControlPropsType".*/
    ...
    */
    return (
        ...
        корневой элемент. Этот компонент возвращает JSX, в котором будет элемент "div", внутри которого будет меняться
...
декларации "Field" ("Ctrl+click" в "WebStorm") и импортировали сюда.*/
    ...
    - "const meta = props.meta". (это мета-данные (тоже создаются элементом "Field" из библиотеки "redux-form"),
    например, внутри них есть "touched" или "error", которые используются в компоненте "FormControl")
    ...
    этому дочернему элементу необходимые для него "props", например "placeholder" (то есть при помощи замыкания будет
...
декларации "Field" ("Ctrl+click" в "WebStorm") и импортировали сюда.*/
    ...
    дочернему элементу необходимые для него "props", например "placeholder" (то есть при помощи замыкания будет доступ
...
формы как аналог "Filed" из библиотеки "redux-form".*/
...
нам нужно, чтобы не было ошибки ниже в "name={name}", так как там по сути ожидается строка.*/
                            ...
                            "Textarea" или "Input" из этого файла). Указали при помощи "React.FC<>", что "props" в этих
                            функциональных компонентах имеют тип "WrappedFieldProps". Этот тип мы нашли в файле
                            декларации "Field" ("Ctrl+click" в "WebStorm") и импортировали сюда.*/
