...
тестов.*/
    ...
    test('correct quantity of pages per a portion', () => {
        ...
        const component = create(<Paginator totalItemsCount={11} pageSize={1} portionSize={10}
                                            currentPage={1} onPageChange={() =>{}}/>); /*Фейково создали компонент
        "Paginator", передав ему 1) общее количество элементов для постраничного вывода, 2) максимальное количество
        элементов на одной странице в постраничном выводе, 3) свойство, которое указывает какое максимальное количество
        номеров страниц в постраничном выводе может отображаться в одной порции таких страниц 4) остальные свойства
        "currentPage" и "onPageChange". То есть у нас всего 11 записей и разрешено по одной записи на страницу, значит
        будет всего 11 страниц. А так как на одну порцию страниц разрешено максимум 10 страниц, то значит будет 2 порции
        страниц - на одной 10 страниц, а на другой только 1 страница. Изначально будет видна первая порция,
        соотвественно должно быть отрисовано 10 "span" элементов с номерами страниц с 1 до 10.*/
    ...
    test('if the quantity of pages is more than 10, then the "NEXT" button should be displayed', () => {
        ...
        будет всего 11 страниц. А так как на одну порцию страниц разрешено максимум 10 страниц, то значит будет 2 порции
        страниц - на одной 10 страниц, а на другой только 1 страница. Изначально будет видна первая порция,
        соотвественно должно быть отрисовано 10 "span" элементов с номерами страниц с 1 до 10. Также поскольку
        количество порций страниц, которое мы имеем, больше текущего номера порции страниц (мы находимся на первой
        порции страниц, а порции две всего), должна отображаться кнопка "следующая порция страниц".*/
        const root = component.root; /*Берем из компонента "root" (видимо элемент содержащий дерево компонента).*/
