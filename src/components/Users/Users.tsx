...
import {
    ...
    getIsNotJustCreated /*Импортируем селектор, который возвращает специальное свойство, которое показывает то, что мы
    успели уже указать данные для фильтрации пользователей в постраничном выводе или нет.*/
...
"props" в этом функциональном компоненте имеют тип "PropsType". Также указали, что экспортируем этот компонент.*/
    ...
    const isNotJustCreated = useSelector(getIsNotJustCreated); /*При помощи хука "useSelector", передав в него селектор
    "getIsNotJustCreated", получаем специальное свойство, которое показывает то, что мы успели уже указать данные для
    фильтрации пользователей в постраничном выводе или нет.*/
    ...
    параметров адресной строки данные при отрисовке компонента, поскольку порядок хуков имеет значение.*/
        ...
        let isJustCreated = true; /*В рамках первого параметра создаем специальное свойство, которое показывает то, что
        мы еще не успели указать данные для фильтрации пользователей в постраничном выводе или успели. Нам это нужно,
        чтобы не затриггерить функцию "push" в компоненте "Users.tsx" при первой отрисовке, то есть когда не указано
        параметров фильтрации пользователей и выбрана первая страница в постраничном выводе по умолчанию, так как
        UI-фреймворк "Ant Design" при взаимодействии с некоторыми элементами интерфейса (например, при нажатии на кнопки
        или при изменении параметров адресной строки) сворачивает все подменю, чего нам не хотелось бы видеть при первой
        отрисовке компонента.*/

        if (!!filter.term) {
            query.term = filter.term;
            isJustCreated = false;
            dispatch(usersAC.toggleIsNotJustCreated(true));
        ...
        указать данные для фильтрации пользователей в постраничном выводе при помощи переменной "isJustCreated" и
        AC "toggleIsNotJustCreated" (последнее для изменения свойства "isNotJustCreated" в "state" и здесь
        соответсвенно).*/
        ...
        фильтрации пользователей в постраничном выводе при помощи переменной "isJustCreated" и AC
        "toggleIsNotJustCreated" (последнее для изменения свойства "isNotJustCreated" в "state" и здесь
        соответсвенно).*/
        ...
        if (currentPage === 1 && isNotJustCreated) {
            query.page = String(currentPage);
            isJustCreated = false;
            dispatch(usersAC.toggleIsNotJustCreated(false));
        ...
        пользователей в постраничном выводе при помощи переменной "isJustCreated", но в этот раз вызываем AC
        "toggleIsNotJustCreated", чтобы изменить свойство "isNotJustCreated" в "state" на "false", чтобы если мы после
        того, как мы совершим следующие операции:
