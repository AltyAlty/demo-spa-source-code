...
"props" в этом функциональном компоненте имеют тип "PropsType". Также указали, что экспортируем этот компонент.*/
    ...
    параметров адресной строки данные при отрисовке компонента, поскольку порядок хуков имеет значение.*/
        ...
        if (!!filter.term) query.term = filter.term; /*Также в рамках первого параметра, если есть свойство "term" в
        объекте "filter", то добавляем его в наш объект "query". При помощи "!!" переводим значение "filter.term" в
        булевый тип.*/

        if (filter.friend !== null) query.friend = String(filter.friend); /*Также в рамках первого параметра, если
        свойство "friend" в объекте "filter" не равно "null", то есть равно "true" или "false", то преобразовываем
        значение этого свойства в строку при помощи функции "String()" и добавляем преобразованное значение в наш
        объект "query".*/

        if (currentPage !== 1) query.page = String(currentPage); /*Также в рамках первого параметра, если номер текущей
        выбранной страницы не равен "1", то сохраняем этот номер, переведя его в строку при помощи функции "String()", в
        наш объект "query".*/

        history.push({ /*Также в рамках первого параметра вызываем функцию "push" объекта "history", который
        возвращается хуком "useHistory", для того, чтобы перенаправить нас на адрес по пути "/users", дописав к этому
        пути параметры адресной строки из свойства "search" объекта "history".*/
            pathname: '/users',
            search: queryString.stringify(query) /*Здесь при помощи функции "stringify()" из библиотеки "querystring" мы
            преобразовываем наш объект "query" в строку, в итоге, например, получая строку
