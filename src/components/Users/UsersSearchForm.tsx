...
type FormType = {
    ...
    пользователей.*/
    friend: 'true' | 'false' | 'null' /*Свойство, которое содержит значение для фильтра, который работает следующим
    образом: если указать в нем "true", то сервер выберет только тех пользователей, которых мы фолловим, если указать
    "false" - только тех, кого мы не фолловим, если указать "null" или вообще не указывать, то выберет всех
    пользователей. Здесь мы специально указали это в виде строковых значений, так как библиотека "formik" работает
    внутри формы со строковыми значениями. Но в самой форме мы делаем преобразования в булевый тип и в "null" для нашего
    "state" и для запросов на сервер.*/
};
    ...
    return (
        <div> {/*Этот элемент "div" и есть наш корневой элемент.*/}
            ...
            "initialValues", "validate" и "onSubmit".*/
                initialValues={{term: '', friend: 'null'}} /*В атрибуте "initialValues", который содержит стартовые
                значения данных формы, которые будут в ней собираться, указали значения пустая строка и "null" для
                свойств "term" и "friend" соответственно.*/
