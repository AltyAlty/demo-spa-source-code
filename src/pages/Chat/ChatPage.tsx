...
/*Создаем тип для "props" компонента "ChatMessages". "Props" в этом компоненте должны обязательно содержать следующие
поля с указанными типами. Все это нужно для указания типа "props" в функциональном компоненте.*/
type ChatMessagesComponentType = {
    wsChannel: WebSocket | null /*WebSocket-канал должен быть типа "WebSocket" или "null", то есть отсутствовать.*/
};

/*Создаем тип для "props" компонента "AddChatMessageForm". "Props" в этом компоненте должны обязательно содержать
следующие поля с указанными типами. Все это нужно для указания типа "props" в функциональном компоненте.*/
type AddChatMessageFormComponentType = {
    wsChannel: WebSocket | null /*WebSocket-канал должен быть типа "WebSocket" или "null", то есть отсутствовать.*/
};
...
*/
const Chat: React.FC = () => {
    const [wsChannel, setWsChannel] = useState<WebSocket | null>(null); /*При помощи деструктуризирующего
    присваивания создали две переменные. Первая переменная будет хранить первый элемент из хука "useState", этот элемент
    будет означать WebSocket-канала (изначально "null", то есть отсутствует). Вторая переменная будет хранить функцию из
    хука "useState", которая будет изменять первый элемент (то есть указывать что является WebSocket-каналом). При
    помощи типизации указали, что первый параметр может быть равен или объекту с типом "WebSocket", или "null", то есть
    отсутствовать.*/

    useEffect(() => { /*Используем здесь хук "useEffect" для того, чтобы при закрытии WebSocket-канала очищать
    память и чтобы, например, при потере интернет-соеденения и его дальнейшего возобновления пытаться пересоздать
    WebSocket-канал.*/
            let ws: WebSocket; /*В рамках первого параметра объявляем переменную "ws" типа "WebSocket", которая в
            дальнейшем в этом хуке "useEffect" будет использоваться для хранения WebSocket-канала.*/

            const closeEventHandler = () => { /*В рамках первого параметра объявляем функцию "closeEventHandler",
            которая будет вызываться при подписке на событие "close" в WebSocket-канале, и вызывать функцию
            "createWSChannel" каждые 3 секунды с целью пересоздания WebSocket-канала.*/
                console.log('All WebSocket-channels closed');
                setTimeout(createWSChannel, 3000);
            };

            function createWSChannel() { /*В рамках первого параметра объявляем функцию "createWSChannel".*/
                ws?.removeEventListener('close', closeEventHandler); /*Если имеется WebSocket-канал, то мы
                отписываемся в нем от события "close". Для отписки должны указать ту же функцию, что и указывали при
                подписке на это событие.*/

                ws?.close(); /*Далее мы выполняем функцию "close()" у WebSocket-канал (если таковой имеется) с целью
                очистки памяти и полного закрытия этого WebSocket-канала.*/

                ws = new WebSocket('wss://social-network.samuraijs.com/handlers/ChatHandler.ashx'); /*Далее таким
                образом создаем WebSocket-канал, указав адрес согласно API сервера. Согласно API сервера в ответ мы
                будем получать объекты, которые содержат следующие свойства:
                ...
                По стандартному HTTP-протоколу клиент делает запросы на сервер, а сервер отправляет ответы клиенту.
                Чтобы делать наоборот нужно применять дополнительные механизмы, например, такие как "SSE" или
                "WebSocket". "WebSocket" - это в каком-то смысле альтернатива протокола "HTTP" (прикладной уровень).
                "WSS" - это безопасная версия протокола "WebSocket", по аналогии с "HTTPS". По протоколу "WebSocket"
                можно передавать два типа данных: текстовой и бинарный. Также нужно помнить, что в браузерах в средствах
                разработчика нельзя замедлить скорость WebSocket-каналов.*/

                ws.addEventListener('close', closeEventHandler); /*Далее подписываемся на событие "close", при
                срабатывании которого будет вызываться функция "closeEventHandler".*/

                setWsChannel(ws); /*При помощи функции "setWsChannel" из хука "useState" выше указываем в локальный
                "state" новый WebSocket-канал. Мы создаем WebSocket-канал в рамках хука "useState", чтобы менялся
                локальный "state" с целью затриггерить перерисовку компонента.*/

                console.log('New WebSocket-channel opened');
            };

            createWSChannel(); /*В рамках первого параметра вызываем функцию "createWSChannel", описанную выше.*/

            return () => { /*Когда мы выйдем из этого компонента, мы должны отписаться от события "close" в
            WebSocket-канале и выполнить функцию "close()" у WebSocket-канал с целью очистки памяти и полного закрытия
            этого WebSocket-канала. Это называется "cleanup" функцией. Для отписки должны указать ту же функцию, что
            указывали при подписке на это событие.*/
    ...
    */
    return (
        <div> {/*Этот элемент "div" и есть наш корневой элемент.*/}
            <ChatMessages wsChannel={wsChannel}/> {/*Отрисовываем компонент "ChatMessages" и передаем в него
            WebSocket-канал через атрибут "wsChannel".*/}
            <AddChatMessageForm wsChannel={wsChannel}/> {/*Отрисовываем компонент "AddChatMessageForm" и передаем в него
            WebSocket-канал через атрибут "wsChannel".*/}
        </div>
    )
};
...
сообщения в чате.
*/
const ChatMessages: React.FC<ChatMessagesComponentType> = ({wsChannel}) => { /*Указываем какие именно
"props" мы получаем, чтобы не писать далее "props.wsChannel". Указали при помощи "React.FC<>", что "props" в этом
функциональном компоненте имеют тип "ChatMessagesComponentType".*/
    ...
    useEffect(() => { /*Используем хук "useEffect", чтобы заполнять чат сообщениями при изменениях в
    WebSocket-канале.*/
            const messageEventHandler = (e: MessageEvent) => { /*В рамках первого параметра объявляем функцию
            "messageEventHandler", которая будет вызываться при подписке на событие "message" в WebSocket-канале, на
            входе получать информацию из WebSocket-канала, которая внутри себя содержит информацию о сообщениях из чата,
            парсить ее в формат "JSON" при помощи "JSON.parse()", чтобы в дальнейшем можно было сохранить эту информацию
            в нашем массиве "chatMessages" под элементами в виде объектов, после этого дописывать эту информацию о
            сообщениях из чата в наш массив "chatMessages" при помощи функции "setChatMessages" из хука "useState" выше.
            Указываем здесь "previousChatMessages" вместо "chatMessages", так как массив "chatMessages" изначально
            ...
            реализации будет подхватываться уже существующий массив и просто дописываться.*/
                console.log('New chat messages from WebSocket-channel received');
            ...
            wsChannel?.addEventListener('message', messageEventHandler); /*В рамках первого параметра подписываемся на
            событие типа "message" в нашем WebSocket-канале (если таковой имеется) при помощи "addEventListener()". При
            срабатывании этого события будет вызываться функция "messageEventHandler", описанная выше.*/

            console.log('WebSocket-channel is: ');
            console.log(wsChannel);

            return () => { /*Когда мы выйдем из этого компонента, мы должны отписаться от события "message" в
            WebSocket-канале (если таковой имеется). Это называется "cleanup" функцией. Для отписки должны указать ту же
            функцию, что указывали при подписке на это событие.*/
                wsChannel?.removeEventListener('message', messageEventHandler);
            };
        },
        [wsChannel] /*Второй параметр это WebSocket-канал. Если он будет как-либо изменяться, то будет срабатывать
        каждый раз все то, что указано в первом параметре.*/
    );
...
Этот компонент подключается в компоненте "App".
*/
const AddChatMessageForm: React.FC<AddChatMessageFormComponentType> = ({wsChannel}) => { /*Указываем какие
именно "props" мы получаем, чтобы не писать далее "props.wsChannel". Указали при помощи "React.FC<>", что "props" в этом
функциональном компоненте имеют тип "AddChatMessageFormComponentType".*/
    ...
    const [readyStatus, setReadyStatus] = useState<'pending' | 'ready'>('pending'); /*При помощи
    деструктуризирующего присваивания создали две переменные. Первая переменная будет хранить первый элемент из хука
    "useState", этот элемент будет означать статус готовности WebSocket-канала (изначально "pending"). Вторая переменная
    будет хранить функцию из хука "useState", которая будет изменять первый элемент (то есть указывать какой статус
    готовности имеет WebSocket-канал). При помощи типизации указали, что первый параметр может быть равен или "pending",
    или "ready".*/

    useEffect(() => { /*Используем хук "useEffect" для вывода в консоль статуса WebSocket-канала при каждом
    изменении этого статуса.*/
        console.log('Status of WebSocket-channel is ' + readyStatus);
        },
        [readyStatus]
    );

    useEffect(() => { /*Используем хук "useEffect", чтобы менять статус готовности WebSocket-канала для отправки
    информации по нему при срабатывании события "open" в этом WebSocket-канале. Этот статус нужен для проверки, которая
    находится в атрибуте "disabled" в элементе "button" ниже.*/
            const openEventHandler = () => { /*В рамках первого параметра объявляем функцию "openEventHandler", которая
            будет вызываться при подписке на событие "open" в WebSocket-канале, и вызывать функцию "setReadyStatus" из
            хука "useState" выше для указания, что WebSocket-канал имеет нужный статус готовности для отправки
            информации по этому WebSocket-каналу.*/
                setReadyStatus('ready');
            };

            wsChannel?.addEventListener('open', openEventHandler); /*В рамках первого параметра если имеется
            WebSocket-канал, то подписываемся в нем на событие "open" и указываем, что при срабатывании этого события
            должна вызываться функция "openEventHandler" и указывать, что WebSocket-канал имеет нужный статус готовности
            для отправки информации по этому WebSocket-каналу.*/

            return () => { /*Когда мы выйдем из этого компонента, мы должны отписаться от события "open" в
            WebSocket-канале (если таковой имеется). Это называется "cleanup" функцией. Для отписки должны указать ту же
            функцию, что указывали при подписке на это событие.*/
                wsChannel?.removeEventListener('open', openEventHandler);
            };
        },
        [wsChannel] /*Второй параметр это WebSocket-канал. Если он будет как-либо изменяться, то будет срабатывать
        каждый раз все то, что указано в первом параметре.*/
    ...
    текст в WebSocket-канал (если таковой имеется) при помощи функции "send()", а после отправки занулять введеный текст
    сообщения в форме.*/
        if (!chatMessage) {
            return;
        };
        ...
        <div> {/*Этот элемент "div" и есть наш корневой элемент.*/}
            ...
            <div>
                <button onClick={addChatMessage}
                        disabled={wsChannel === null || readyStatus !== 'ready'}> {/*Отрисовываем элемент "button". При
                        нажатии на кнопку будет срабатывать функция "addChatMessage", которую мы создали выше, с целью
                        отправки введенного сообщения в WebSocket-канал, и для дальнейшей очистки этого текста сообщения
                        после отправки этого сообщения. Также поскольку есть вероятность того, что мы попробуем
                        отправить сообщение раньше, чем установится WebSocket-канал, то мы отключаем кнопку на случай
                        если отсуствует WebSocket-канал или этот канал еще не в готовом состоянии для отправки
                        информации по нему.*/}
                    Send
                </button>
            </div>
