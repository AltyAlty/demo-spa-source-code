...
import {AppStateType, InferActionsTypes} from './redux-store'; /*Импортируем типы.*/
...
"ActionsType", созданный нами ниже.*/
    switch (action.type) {
        ...
        case 'react-samurai-01/users-reducer/SET-USERS':
        ...
        case 'react-samurai-01/users-reducer/SET-CURRENT-PAGE':
        ...
        case 'react-samurai-01/users-reducer/SET-TOTAL-USERS-COUNT':
        ...
        case 'react-samurai-01/users-reducer/TOGGLE-IS-FETCHING':
        ...
        case 'react-samurai-01/users-reducer/TOGGLE-IS-FOLLOWING-IN-PROGRESS':
...
типов для каждого AC в обмен на один общий для них тип, который мы создали выше.*/
    ...
    "userID", которое дожно быть числом.*/
        type: 'react-samurai-01/users-reducer/FOLLOW', /*Обязательно свойство "type" для AC.*/
    ...
    "userID", которое дожно быть числом.*/
        type: 'react-samurai-01/users-reducer/UNFOLLOW', /*Обязательно свойство "type" для AC.*/
    ...
    импортированного сюда.*/
        type: 'react-samurai-01/users-reducer/SET-USERS', /*Обязательно свойство "type" для AC.*/
        users /*Это равносильно "users: users". Данные по пользователям для постраничного вывода пользователей,
        полученные с сервера.*/
    } as const),

    /*Временно скрыли здесь AC "setCurrentPage", так как нам пока еще нужна его старая реализация (отдельно AC
    и отдельно тип для этого AC, смотри выше).*/
    ...
    входе получает "totalUsersCount", которое дожно быть числом.*/
        type: 'react-samurai-01/users-reducer/SET-TOTAL-USERS-COUNT', /*Обязательно свойство "type" для AC.*/
    ...
    компонент-заглушка "Preloader". На входе получает "isFetching", которое дожно быть булева типа.*/
        type: 'react-samurai-01/users-reducer/TOGGLE-IS-FETCHING', /*Обязательно свойство "type" для AC.*/
    ...
    то кнопка для анфолловинга/фолловинга будет отключена.*/
        type: 'react-samurai-01/users-reducer/TOGGLE-IS-FOLLOWING-IN-PROGRESS', /*Обязательно свойство "type" для AC.*/
...
const _followUnfollowFlow = async (dispatch: DispatchType, /*На входе принимает "dispatch", который должен быть типа
    ...
    dispatch(usersAC.toggleIsFollowingInProgress(true, id)); /*Указываем, что анфолловим/фолловим
    ...
    dispatch(usersAC.toggleIsFollowingInProgress(false, id)); /*Указываем, что больше не
    анфолловим/фолловим какого-то пользователя.*/
}
...
"dispatch", "getState()" и дополнительные аргументы.*/
    _followUnfollowFlow(dispatch, id, usersAPI.unfollow.bind(usersAPI), usersAC.unfollowSuccess); /*Так как мы
...
возвращает TC, то есть "thunk", мы также типизировали, что в "thunk" будет передаваться дальше, то есть те самые
"dispatch", "getState()" и дополнительные аргументы.*/
    _followUnfollowFlow(dispatch, id, usersAPI.follow.bind(usersAPI), usersAC.followSuccess); /*Так как мы берем
