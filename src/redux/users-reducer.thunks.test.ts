...
тестов.*/
    ...
    так как внутри мы используем асинхронные "thunks".*/
        ...
        /*Шаги теста.*/
        usersAPIMock.follow.mockReturnValue(Promise.resolve(followUnfollowFromUsersAPIResult)); /*Таким образом указали,
        что если запрос "follow" будет вызываться из "usersAPIMock", то вместо реальной работы он должен всегда
        возвращать в качестве результата промис, который резольвится объектом "followUnfollowFromUsersAPIResult",
        который мы создали выше (хотя можно было указать просто объект "followUnfollowFromUsersAPIResult" без
        промиса). Ранее этот кусок кода был выше вне тестов, но сейчас его нужно помещать внутрь самих тестов, так как,
        возможно из-за изменений в версиях "Node.js", работа промисов немного изменилась.*/
    ...
    так как внутри мы используем асинхронные "thunks".*/
        ...
        /*Шаги теста.*/
        usersAPIMock.unfollow.mockReturnValue(Promise.resolve(followUnfollowFromUsersAPIResult)); /*Таким образом
        указали, что если запрос "unfollow" будет вызываться из "usersAPIMock", то вместо реальной работы он должен
        всегда возвращать в качестве результата промис, который резольвится объектом "followUnfollowFromUsersAPIResult",
        который мы создали выше (хотя можно было указать просто объект "followUnfollowFromUsersAPIResult" без
        промиса). Ранее этот кусок кода был выше вне тестов, но сейчас его нужно помещать внутрь самих тестов, так как,
        возможно из-за изменений в версиях "Node.js", работа промисов немного изменилась.*/
